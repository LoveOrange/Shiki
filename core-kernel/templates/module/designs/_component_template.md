...
| :--- | :--- | :--- | :--- |
| model/persistence/ACL scope | baseline/source components, services, repositories, supports, and adapters checked | `reuse/extend/modify/add/MANUAL_DECISION` with reason | Every `add` names source evidence and why reuse/extension is not correct; use `N/A` when there are no additions. |
