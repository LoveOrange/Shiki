...
2. If a leaf spec `§0 Baseline Delta` contains `reuse`, load the referenced baseline snippet and record it in `Source Specs`.
3. Extract facts into `code_contract.md`: Entities, DTOs, Interfaces, State Transitions, Error Codes, Non-functional Rules.
4. Add `[ ]` confirmation markers for each implementable fact.
5. Set `Contract Version: v1` unless a version is already required by the plan.
6. Do not add implementation detail not present in L2 specs or explicitly reused baseline refs.
